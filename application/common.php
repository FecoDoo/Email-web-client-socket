...
{
    $mailLink = "{{$host}:{$port}}INBOX"; //imap连接地址：不同主机地址不同
    $mailUser = $user; //邮箱用户名
    $mailPass = $pwd; //邮箱密码
    ...
    for ($i = 1; $i <= $totalrows; $i++) {
        ...
        $headlist = matchMailHead($headers);
...
function matchMailHead($str)
{
    ...
    if (preg_match('/From:\s+(.*?)+\s</', $str, $a)) {
        $from = preg_replace('/</', '', preg_replace('/From:\s/', '', $a));
        $result = $from[0];
        if (!empty($result)) {
            array_push($headList, $result);
        } else {
            array_push($headList, 'Unknown');
        }
    }

    if (preg_match('/Date:\s+(.*?)+\sTo/i', $str, $a)) {
        $date = preg_replace('/To/', '', preg_replace('/Date:\s/', '', $a));
        $result = $date[0];
        if (!empty($result)) {
            array_push($headList, $result);
        } else {
    ...
    if (preg_match('/Subject:\s+(.*?)+\sMessage/i', $str, $a)) {
        $subject = preg_replace('/Message/', '', preg_replace('/Subject:\s/', '', $a));
        $result = $subject[0];
        if (!empty($result)) {
            array_push($headList, $result);
        } else {
